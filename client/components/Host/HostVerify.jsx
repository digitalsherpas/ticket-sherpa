...
export default class HostVerify extends Component {
  ...
  hostVerifyTicket(eventContractAddress, userWalletAddress) {
    ...
    this.props.updateTicketVerificationStarted();

    var context = this;
    const test = contract.verifyAttendee(account, {from: account}, function (err, result) {
      if (!err) {
        context.props.updateTicketVerificationStatus(result);
      }
    });
  }
  ...
  componentWillMount() {
    ...
    this.hostVerifyTicket(query.eventContractAddress, query.userWalletAddress);
  }
